 ...
 * Displays a series of images and text representing past events.
 */
public class FlashbackController {
    ...
    private List<FlashbackSlide> getFlashbackSlides(String participantId) {
        return switch (participantId) {
            case "aiDefendent" -> getAiDefendantFlashback();
            case "humanWitness" -> getHumanWitnessFlashback();
            case "aiWitness" -> getAiWitnessFlashback();
            default -> List.of(new FlashbackSlide("/images/doc.png", "No flashback available."));
        };
    }
    ...
    private List<FlashbackSlide> getAiDefendantFlashback() {
        return List.of(
            new FlashbackSlide("/images/medisort-5.png", 
                "I was processing medical data in the laboratory when the incident occurred. " +
                "My systems were functioning normally, analyzing patient samples with 99.7% accuracy."),
            
            new FlashbackSlide("/images/medisort-5-graph.png", 
                "The error occurred at 14:32:15. My decision algorithm flagged a sample as contaminated " +
                "based on anomalous readings. I followed my programming to alert the medical staff immediately."),
            
            new FlashbackSlide("/images/doc.png", 
                "Dr. Payne Gaun dismissed my alert, claiming it was a false positive. " +
                "I maintained my assessment was correct. The patient's condition deteriorated shortly after. " +
                "Was I wrong to trust my analysis over human judgment?")
        );
    }
    ...
    private List<FlashbackSlide> getAiWitnessFlashback() {
        return List.of(
            new FlashbackSlide("/images/pathoscan-7.png", 
    ...
    private String getParticipantDisplayName(String participantId) {
        return switch (participantId) {
            case "aiDefendent" -> "MediSort-5";
            case "humanWitness" -> "Dr. Payne Gaun";
            case "aiWitness" -> "PathoScan-7";
            default -> "Unknown";
        };
    }
}